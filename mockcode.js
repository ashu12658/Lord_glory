...
//           fetch(`${process.env.REACT_APP_API_URL}/agents/orders-count`, {
//             headers: { Authorization: `Bearer ${token}` }
//           }),
//           fetch(`${process.env.REACT_APP_API_URL}/agents/get-coupen-user`, {
//             headers: { Authorization: `Bearer ${token}` }
//           }),
//           fetch(`${process.env.REACT_APP_API_URL}/agents/get-balance`, {
//             headers: { Authorization: `Bearer ${token}` }
//           })
//         ]);
...
//                 }}
//               >
//                 <FiAlertTriangle size={14} /> Under Development
//               </motion.span>
//             </motion.h1>
//             <motion.p
//               initial={{ opacity: 0 }}
...
//               }}
//             >
//               Manage your Lord Glory products, track orders, and monitor your commissions all in one place. More features will be available in a couple of days.
//             </motion.p>
//           </div>
...
const { StandardCheckoutPayRequest,Env,StandardCheckoutClient,OrderStatusResponse } =require ('pg-sdk-node');
...
router.post('/initiate', async (req, res) => {
  try {
    ...
    // Format the products array (if needed for later use)
    const formattedProducts = products.map(p => ({
      product: p.productId,
      quantity: p.quantity
    }));

    // Create the order in your database (order status will remain "pending" until payment is complete)
    const newOrder = new Order({
      user: userId,
      product: formattedProducts,
      totalAmount: finalAmount,
    ...
    // For example, you can prefix with "MT" and use the order ID (ensuring it's below 35 characters)
    const merchantTransactionId = "MT" + savedOrder._id.toString().slice(-12);
    
    
    // Prepare the PhonePe API payload
    // Note: Amount must be in paise, so multiply by 100.
    const phonePePayload = {
      merchantId: MERCHANT_ID,
      merchantTransactionId, // Unique transaction id for PhonePe
      merchantUserId: userId,
      amount: Math.round(finalAmount * 100), // Convert rupees to paise
      redirectUrl: `${FRONTEND_URL}/payment-confirmation?orderId=${savedOrder._id}&amount=${finalAmount}&coupon=${couponCode || ''}`,
      // Optional: Include a callbackUrl if required by your flow. If not, you can omit or use the redirectUrl.
    ...
    // Generate the X-VERIFY header as per PhonePe docs:
    // SHA256(base64EncodedPayload + "/pg/v1/pay" + SALT_KEY) + "###" + SALT_INDEX
    const stringToHash = payloadBase64 + "/checkout/v2/pay"
    const sha256Hash = crypto.createHash('sha256').update(stringToHash).digest('hex');
    const xVerifyChecksum = `${sha256Hash}###${SALT_INDEX}`;
    console.log("X-VERIFY checksum:", xVerifyChecksum);
  console.log("PHONEPE_UAT_URL: ",PHONEPE_UAT_URL);
  
console.log("All env vars:", process.env);

    // Make the POST request to PhonePe's UAT endpoint
    ...
    // Expecting the PhonePe response to have a redirectInfo URL
    if (phonePeResponse.data && phonePeResponse.data.data && phonePeResponse.data.data.instrumentResponse) {
      // Option 1: Redirect the user to the payment URL immediately
      // return res.redirect(phonePeResponse.data.data.instrumentResponse.redirectInfo.url);

      // Option 2: Send the URL and order details back to the frontend for further handling.
      return res.json({
        success: true,
      ...
      return res.status(500).json({
        success: false,
        message: "PhonePe response missing instrumentResponse data"
      });
    }
  } catch (error) {
    console.error("Error creating order:", error);
...
//Testing Credentials
/* 
const MERCHANT_ID = process.env.TEST_MERCHANT_ID
const SALT_KEY = process.env.TEST_SALT_KEY
const SALT_INDEX = process.env.SALT_INDEX || "1";
const PHONEPE_UAT_URL = process.env.PHONEPE_UAT_URL
...
const SALT_KEY = process.env.SALT_KEY
const SALT_INDEX = process.env.SALT_INDEX || "1";
const PHONEPE_UAT_URL = process.env.PHONEPE_PROD_URL
...
const client = StandardCheckoutClient.getInstance(MERCHANT_ID, SALT_KEY, clientVersion, env);
...
const merchantOrderId =  savedOrder._id;
const redirectUrl = `${FRONTEND_URL}/payment-confirmation?orderId=${merchantOrderId}&amount=${finalAmount}&coupon=${couponCode || ''}&token=${encodeURIComponent(token)}`;
  console.log("🧾 Creating PhonePe payment request...");
console.log("Merchant ID:", MERCHANT_ID);
console.log("Merchant Order ID:", merchantOrderId);
console.log("Redirect URL:", redirectUrl);
console.log("Amount (paise):", Math.round(finalAmount * 100));
...
client.pay(request)
  .then((response) => {
    ...
    console.log("🔁 Redirecting user to PhonePe checkout page:", checkoutPageUrl);
 ...
 res.status(200).json({
  ...
  .catch((error) => {
    console.error("❌ Error while initiating PhonePe payment");
    console.error("Error message:", error.message);
    console.error("Error details:", error);
    res.status(500).json({ error: "Failed to initiate PhonePe payment", message: error.message });
  });
...
router.post('/checkOrderStatus', async (req, res) => {
  try {
   
   const order = await Order.findById(req.body.orderId);
    if (!order) return res.status(404).json({ error: "Order not found" });
		const merchantOrderId = req.body.orderId; //created at the time of order creation
    ...
    return res.status(500).json({ message: "Failed to Check Order", error: error.message });
  }
});
